 ...
 ****************************************************/
#ifndef ADAFRUIT_FONA_3G_H
#define ADAFRUIT_FONA_3G_H
...
class Adafruit_FONA_3G : public FONAStreamType {
 public:
  Adafruit_FONA_3G(int8_t r);
  ...
  boolean HTTP_GET_start_From_String(String *url, uint16_t *status, uint16_t *datalen);
  ...
  boolean sendCheckReply(String send, FONAFlashStringPtr reply, uint16_t timeout = FONA_DEFAULT_TIMEOUT_MS);  
  boolean sendCheckReplyFromStringPtr(String* send, FONAFlashStringPtr reply, uint16_t timeout = FONA_DEFAULT_TIMEOUT_MS);  
