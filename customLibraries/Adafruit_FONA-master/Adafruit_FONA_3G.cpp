...
Adafruit_FONA_3G::Adafruit_FONA_3G(int8_t rst)
{
  _rstpin = rst;
...
uint8_t Adafruit_FONA_3G::type(void) {
  return _type;
}

boolean Adafruit_FONA_3G::begin(Stream &port) {
  mySerial = &port;
...
/********* Serial port ********************************************/
boolean Adafruit_FONA_3G::setBaudrate(uint16_t baud) {
  return sendCheckReply(F("AT+IPREX="), baud, ok_reply);
}

/********* Real Time Clock ********************************************/

boolean Adafruit_FONA_3G::readRTC(uint8_t *year, uint8_t *month, uint8_t *date, uint8_t *hr, uint8_t *min, uint8_t *sec) {
  uint16_t v;
...
boolean Adafruit_FONA_3G::enableRTC(uint8_t i) {
...
boolean Adafruit_FONA_3G::getBattPercent(uint16_t *p) {
...
uint8_t Adafruit_FONA_3G::getSIMCCID(char *ccid) {
  getReply(F("AT+CCID"));
...
uint8_t Adafruit_FONA_3G::getIMEI(char *imei) {
  getReply(F("AT+GSN"));
...
uint8_t Adafruit_FONA_3G::getNetworkStatus(void) {
  uint16_t status;
...
uint8_t Adafruit_FONA_3G::getRSSI(void) {
  uint16_t reply;
...
boolean Adafruit_FONA_3G::setAudio(uint8_t a) {
...
uint8_t Adafruit_FONA_3G::getVolume(void) {
  uint16_t reply;
...
boolean Adafruit_FONA_3G::setVolume(uint8_t i) {
  return sendCheckReply(F("AT+CLVL="), i, ok_reply);
}


boolean Adafruit_FONA_3G::playDTMF(char dtmf) {
  char str[4];
  str[0] = '\"';
  str[1] = dtmf;
  str[2] = '\"';
...
boolean Adafruit_FONA_3G::setMicVolume(uint8_t a, uint8_t level) {
  // 0 is headset, 1 is external audio
  if (a > 1) return false;
...
boolean Adafruit_FONA_3G::FMradio(boolean onoff, uint8_t a) {
  if (! onoff) {
...
boolean Adafruit_FONA_3G::tuneFMradio(uint16_t station) {
...
boolean Adafruit_FONA_3G::setFMVolume(uint8_t i) {
...
int8_t Adafruit_FONA_3G::getFMVolume() {
  uint16_t level;
...
int8_t Adafruit_FONA_3G::getFMSignalLevel(uint16_t station) {
  // Fail if FM station is outside allowed range.
...
boolean Adafruit_FONA_3G::setPWM(uint16_t period, uint8_t duty) {
...
/********* CALL PHONES **************************************************/
boolean Adafruit_FONA_3G::callPhone(char *number) {
...
uint8_t Adafruit_FONA_3G::getCallStatus(void) {
...
void Adafruit_FONA_3G::onIncomingCall() {

  DEBUG_PRINT(F("> ")); DEBUG_PRINTLN(F("Incoming call..."));

  Adafruit_FONA_3G::_incomingCall = true;
}

boolean Adafruit_FONA_3G::_incomingCall = false;

boolean Adafruit_FONA_3G::callerIdNotification(boolean enable, uint8_t interrupt) {
  if(enable){
...
boolean Adafruit_FONA_3G::incomingCallNumber(char* phonenum) {
  //+CLIP: "<incoming phone number>",145,"",0,"",0
  if(!Adafruit_FONA_3G::_incomingCall)
    return false;
  ...
  Adafruit_FONA_3G::_incomingCall = false;
  return true;
}

/********* SMS **********************************************************/

uint8_t Adafruit_FONA_3G::getSMSInterrupt(void) {
  uint16_t reply;
...
boolean Adafruit_FONA_3G::setSMSInterrupt(uint8_t i) {
  return sendCheckReply(F("AT+CFGRI="), i, ok_reply);
}

int8_t Adafruit_FONA_3G::getNumSMS(void) {
  uint16_t numsms;
...
// printouts!
boolean Adafruit_FONA_3G::readSMS(uint8_t i, char *smsbuff,
...
// otherwise false.
boolean Adafruit_FONA_3G::getSMSSender(uint8_t i, char *sender, int senderlen) {
...
boolean Adafruit_FONA_3G::sendSMS(char *smsaddr, char *smsmsg) {
  if (! sendCheckReply(F("AT+CMGF=1"), ok_reply)) return false;
...
boolean Adafruit_FONA_3G::deleteSMS(uint8_t i) {
    if (! sendCheckReply(F("AT+CMGF=1"), ok_reply)) return false;
  // read an sms
...
boolean Adafruit_FONA_3G::sendUSSD(char *ussdmsg, char *ussdbuff, uint16_t maxlen, uint16_t *readlen) {
...
boolean Adafruit_FONA_3G::enableNetworkTimeSync(boolean onoff) {
  if (onoff) {
...
boolean Adafruit_FONA_3G::enableNTPTimeSync(boolean onoff, FONAFlashStringPtr ntpserver) {
  if (onoff) {
...
boolean Adafruit_FONA_3G::getTime(char *buff, uint16_t maxlen) {
...
boolean Adafruit_FONA_3G::enableGPS(boolean onoff) {
  ...
  if (! Adafruit_FONA_3G::sendParseReply(F("AT+CGPS?"), F("+CGPS: "), &state) )
    return false;
...
int8_t Adafruit_FONA_3G::GPSstatus(void) {
  if (_type == FONA808_V2) {
...
uint8_t Adafruit_FONA_3G::getGPS(uint8_t arg, char *buffer, uint8_t maxbuff) {
  int32_t x = arg;
...
boolean Adafruit_FONA_3G::enableGPSNMEA(uint8_t i) {
...
uint8_t Adafruit_FONA_3G::GPRSstate(void) {
  uint16_t state;
...
void Adafruit_FONA_3G::setGPRSNetworkSettings(FONAFlashStringPtr apn,
...
boolean Adafruit_FONA_3G::getGSMLoc(uint16_t *errorcode, char *buff, uint16_t maxlen) {
...
boolean Adafruit_FONA_3G::getGSMLoc(float *lat, float *lon) {
...
boolean Adafruit_FONA_3G::TCPconnect(char *server, uint16_t port) {
  flushInput();
...
boolean Adafruit_FONA_3G::TCPclose(void) {
...
boolean Adafruit_FONA_3G::HTTP_init() {
  return sendCheckReply(F("AT+CHTTPSSTART"), ok_reply);
}

boolean Adafruit_FONA_3G::HTTP_term() {
  return sendCheckReply(F("AT+CHTTPSSTOP"), ok_reply);
}

void Adafruit_FONA_3G::HTTP_para_start(FONAFlashStringPtr parameter,
                                    boolean quoted) {
  flushInput();
...
boolean Adafruit_FONA_3G::HTTP_para_end(boolean quoted) {
  if (quoted)
    mySerial->println('"');
  else
    mySerial->println();
...
boolean Adafruit_FONA_3G::HTTP_para(FONAFlashStringPtr parameter,
                                 const char *value) {
...
boolean Adafruit_FONA_3G::HTTP_data(uint32_t size, uint32_t maxTime) {
  flushInput();
...
boolean Adafruit_FONA_3G::HTTP_action(uint8_t method, uint16_t *status,
...
boolean Adafruit_FONA_3G::HTTP_readall(uint16_t *datalen) {
...
boolean Adafruit_FONA_3G::HTTP_ssl(boolean onoff) {
...
boolean Adafruit_FONA_3G::HTTP_GET_start_From_String(String *url,
              uint16_t *status, uint16_t *datalen){
                
  uint16_t doubleSlashIndex = (*url).indexOf("://");
  if(-1 == doubleSlashIndex) {
    return false;
  }
  
  uint16_t nextSlashIndex = (*url).indexOf("/", doubleSlashIndex + 3);
  if(-1 == nextSlashIndex) {
    return false;
  }  
  
  String webAddr = (*url).substring(doubleSlashIndex + 3, nextSlashIndex);
  String getRequest = F("GET ");
  getRequest += (*url).substring(nextSlashIndex);
  getRequest += F(" HTTP/1.1\r\n");
  getRequest += F("Host: ");
  getRequest += webAddr;
  getRequest += F("\r\n\r\n");
  ...
  String webAddrRequest = F("AT+CHTTPSOPSE=");
  webAddrRequest += F("\"");
  webAddrRequest += webAddr;
  webAddrRequest += F("\"");
  webAddrRequest += F(",80,1");

  if(!sendCheckReplyFromStringPtr(&webAddrRequest, ok_reply, 30000)) {
  ...
  String getRequestRequest = F("AT+CHTTPSSEND=");
  getRequestRequest += String(getRequest.length());
  
  if(!sendCheckReplyFromStringPtr(&getRequestRequest, F(">"))) {
    sendCheckReply(F("AT+CHTTPSCLSE"), ok_reply);
    return false;
  }  
  ...
  if(!sendCheckReplyFromStringPtr(&getRequest, ok_reply)) {
    sendCheckReply(F("AT+CHTTPSCLSE"), ok_reply);
    return false;
  }    
...
boolean Adafruit_FONA_3G::HTTP_GET_start(char *url,
              uint16_t *status, uint16_t *datalen){
  ...
  uint16_t doubleSlashIndex = urlStr.indexOf("://");
  if(-1 == doubleSlashIndex) {
    return false;
  }
  
  uint16_t nextSlashIndex = urlStr.indexOf("/", doubleSlashIndex + 3);
  if(-1 == nextSlashIndex) {
    return false;
  }  
  
  String webAddr = urlStr.substring(doubleSlashIndex + 3, nextSlashIndex);
  String getRequest = F("GET ");
  getRequest += urlStr.substring(nextSlashIndex);
  getRequest += F(" HTTP/1.1\r\n");
  getRequest += F("Host: ");
  getRequest += webAddr;
  getRequest += F("\r\n\r\n");
  ...
  if(!sendCheckReply(webAddrRequest, ok_reply, 30000)) {
    sendCheckReply(F("AT+CHTTPSCLSE"), ok_reply);
    return false;
  }
  ...
  if(!sendCheckReply(getRequestRequest, F(">"))) {
    sendCheckReply(F("AT+CHTTPSCLSE"), ok_reply);
    return false;
  }  
  ...
  if(!sendCheckReply(getRequest, ok_reply)) {
    sendCheckReply(F("AT+CHTTPSCLSE"), ok_reply);
    return false;
  }    
...
boolean Adafruit_FONA_3G::HTTP_POST_start(char *url,
...
void Adafruit_FONA_3G::HTTP_POST_end(void) {
  HTTP_term();
}

void Adafruit_FONA_3G::setUserAgent(FONAFlashStringPtr useragent) {
  this->useragent = useragent;
}

void Adafruit_FONA_3G::setHTTPSRedirect(boolean onoff) {
  httpsredirect = onoff;
}

/********* HTTP HELPERS ****************************************/

boolean Adafruit_FONA_3G::HTTP_setup(char *url) {
  // Handle any pending
  HTTP_term();
...
boolean Adafruit_FONA_3G::expectReply(FONAFlashStringPtr reply,
                                   uint16_t timeout) {
  readline(timeout);
...
inline int Adafruit_FONA_3G::available(void) {
  return mySerial->available();
}

inline size_t Adafruit_FONA_3G::write(uint8_t x) {
  return mySerial->write(x);
}

inline int Adafruit_FONA_3G::read(void) {
  return mySerial->read();
}

inline int Adafruit_FONA_3G::peek(void) {
  return mySerial->peek();
}

inline void Adafruit_FONA_3G::flush() {
  mySerial->flush();
}

void Adafruit_FONA_3G::flushInput() {
...
uint16_t Adafruit_FONA_3G::readRaw(uint16_t b) {
  uint16_t idx = 0;
...
uint8_t Adafruit_FONA_3G::readline(uint16_t timeout, boolean multiline) {
  uint16_t replyidx = 0;
...
uint8_t Adafruit_FONA_3G::getReply(char *send, uint16_t timeout) {
  flushInput();
...
uint8_t Adafruit_FONA_3G::getReply(FONAFlashStringPtr send, uint16_t timeout) {
  flushInput();
...
uint8_t Adafruit_FONA_3G::getReply(FONAFlashStringPtr prefix, char *suffix, uint16_t timeout) {
  flushInput();
...
// Send prefix, suffix, and newline. Return response (and also set replybuffer with response).
uint8_t Adafruit_FONA_3G::getReply(FONAFlashStringPtr prefix, int32_t suffix, uint16_t timeout) {
  flushInput();
...
uint8_t Adafruit_FONA_3G::getReply(FONAFlashStringPtr prefix, int32_t suffix1, int32_t suffix2, uint16_t timeout) {
  flushInput();
...
// Send prefix, ", suffix, ", and newline. Return response (and also set replybuffer with response).
uint8_t Adafruit_FONA_3G::getReplyQuoted(FONAFlashStringPtr prefix, FONAFlashStringPtr suffix, uint16_t timeout) {
  flushInput();
...
boolean Adafruit_FONA_3G::sendCheckReply(char *send, char *reply, uint16_t timeout) {
...
boolean Adafruit_FONA_3G::sendCheckReply(FONAFlashStringPtr send, FONAFlashStringPtr reply, uint16_t timeout) {
	if (! getReply(send, timeout) )
		return false;
...
boolean Adafruit_FONA_3G::sendCheckReply(char* send, FONAFlashStringPtr reply, uint16_t timeout) {
  if (! getReply(send, timeout) )
	  return false;
  return (prog_char_strcmp(replybuffer, (prog_char*)reply) == 0);
}

boolean Adafruit_FONA_3G::sendCheckReply(String send, FONAFlashStringPtr reply, uint16_t timeout) {
  char sendBuffer[255];
  
  send.toCharArray(sendBuffer, sizeof(sendBuffer), 0);

  if (! getReply(sendBuffer, timeout) )
	  return false;
  return (prog_char_strcmp(replybuffer, (prog_char*)reply) == 0);
}
...
boolean Adafruit_FONA_3G::sendCheckReplyFromStringPtr(String* send, FONAFlashStringPtr reply, uint16_t timeout) {
  char sendBuffer[255];
...
boolean Adafruit_FONA_3G::sendCheckReply(FONAFlashStringPtr prefix, char *suffix, FONAFlashStringPtr reply, uint16_t timeout) {
  getReply(prefix, suffix, timeout);
  return (prog_char_strcmp(replybuffer, (prog_char*)reply) == 0);
}

// Send prefix, suffix, and newline.  Verify FONA response matches reply parameter.
boolean Adafruit_FONA_3G::sendCheckReply(FONAFlashStringPtr prefix, int32_t suffix, FONAFlashStringPtr reply, uint16_t timeout) {
  getReply(prefix, suffix, timeout);
  return (prog_char_strcmp(replybuffer, (prog_char*)reply) == 0);
}

// Send prefix, suffix, suffix2, and newline.  Verify FONA response matches reply parameter.
boolean Adafruit_FONA_3G::sendCheckReply(FONAFlashStringPtr prefix, int32_t suffix1, int32_t suffix2, FONAFlashStringPtr reply, uint16_t timeout) {
  getReply(prefix, suffix1, suffix2, timeout);
  return (prog_char_strcmp(replybuffer, (prog_char*)reply) == 0);
}

// Send prefix, ", suffix, ", and newline.  Verify FONA response matches reply parameter.
boolean Adafruit_FONA_3G::sendCheckReplyQuoted(FONAFlashStringPtr prefix, FONAFlashStringPtr suffix, FONAFlashStringPtr reply, uint16_t timeout) {
...
// response.
boolean Adafruit_FONA_3G::parseReplyQuoted(FONAFlashStringPtr toreply,
